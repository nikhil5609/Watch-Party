import { preloadFFmpeg } from "../../Utils/loadffmpeg";
...
import { optimizeVideoClient } from "../../Utils/videoOptimized";
...
const Library = () => {
  ...
  useEffect(() => {
    console.log("Loading Start FFMPEG");
    preloadFFmpeg();
    console.log("Loading END FFMPEG");
  }, []);
  ...
  const handleUpload = async () => {
    ...
    let fileToUpload;
    try {
      fileToUpload = await optimizeVideoClient(selectedFile);
    } catch (err) {
      console.warn(
        "Client-side optimize failed, uploading raw for backend to handle:",
        err,
      );
    }
    ...
    try {
      ...
      alert("Upload failed. Please try again.",err);
    } finally {
      setUploading(false);
    }
  };
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div className="space-y-6">
                  <h2 className="text-3xl font-black text-white">
                    Upload to <span className="text-red-600">Cloud.</span>
                  </h2>
